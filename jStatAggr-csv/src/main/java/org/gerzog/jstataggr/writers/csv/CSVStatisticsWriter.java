...
public class CSVStatisticsWriter implements IStatisticsWriter {
	...
	protected static class WritersCache {
		...
		public void allWriters(final Consumer<ICsvBeanWriter> action) {
			cache.values().forEach(
					statisticsWriters -> {
						statisticsWriters.values().forEach(
								writer -> action.accept(writer));
					});
		}
	...
	protected void finishUp(final WritersCache cache) {
		cache.allWriters(writer -> IOUtils.closeQuietly(writer));
	}
	...
	protected String[] getFieldNames(final Class<?> clazz,
		...
		final List<String> result = BeanUtils.getBeanProperties(clazz, false)
				.stream()
				.filter(name -> !config.getExcludedFields().contains(name))
				.sorted(config.getFieldComparator())
				.collect(Collectors.toList());
	...
	protected BeanInfo getBeanInfo(final Object object) {
		return propogate(() -> Introspector.getBeanInfo(object.getClass()));
	}
}
